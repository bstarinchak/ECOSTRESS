...
# possible_Dates=filter(calib_curv, mean.F14C>1.220057 & mean.F14C<1.226257)
# 
# str(df)
# df=C14_sugars_May_2018_averaged
# df$calibrated_dates=NA
# df=as.data.frame(df)
# i=9
# for(i in 1:nrow(df)){
#   possible_Dates=filter(calib_curv, mean.F14C>(df[i,c("F14C")]-df[i,c("err_F14C")]) & mean.F14C<(df[i,c("F14C")]+df[i,c("err_F14C")]))
#   if(nrow(possible_Dates)==0){
#     df$calibrated_dates[i]=2253.3-230.8*df$F14C[i]
#   }else{
#     df$calibrated_dates[i]=mean(possible_Dates[,1])
...
#### July 2018 for Respiration and NSC 14C values
...
######## sweeden trees (roots and branches)
